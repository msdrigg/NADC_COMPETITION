...
for i in range(num_pairs):
    pairs.append(tuple(map(lambda a: int(a), input().split())))
...
if len(matching) < len(pairs):
    print("impossible")
else:
    for idx_pair in matching:
        expression = '*'
        ans = matching[(idx_pair)]
        idx = idx_pair[0]
        pair = idx_pair[1]
        ...
        pairs[idx] = (pairs[idx], expression, ans)


    for pair, expression, ans in pairs:
        print(f"{pair[0]} {expression} {pair[1]} = {ans}")